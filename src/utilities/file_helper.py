...
def write_last_reference_image_coordinates(
    reference_image_name: str, coordinates: list
):
...
def write_last_reference_image_parameters(
    reference_image_name: str, values_of_parameters: tuple[int]
):
    file_path = paths.SRC_PARAMETERS_OF_REFERENCE_IMAGES.joinpath(
        "".join([reference_image_name, ".txt"])
    )
    with file_path.open("w", encoding="utf-8") as file:
