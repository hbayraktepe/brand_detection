...
from tkinter import HORIZONTAL, Label, Scale, Toplevel, ttk
...
from src.utilities.file_helper import (read_last_reference_image_coordinates,
...
class BrandDetectionApp:
    def __init__(
        ...
        self.selected_reference_image_path = paths.SRC_ASSETS_DIR.joinpath(
            "selected_reference_image.png"
        )
        ...
        self.product_image_path = paths.SRC_PRODUCT_IMAGES_DIR.joinpath(
            "product_image.png"
        )
        ...
        self.saved_reference_images_combobox.bind(
            "<<ComboboxSelected>>", self.on_select_combobox
        )
        ...
        self.reference_image_area_select_buttons_frame.grid(
            row=1, column=0, sticky="W", padx=50, pady=20
        )
        ...
        self.reference_select_image_button_frame.grid(
            row=2, column=0, sticky="W", padx=50, pady=20
        )
        ...
        self.product_image_button_frame.grid(
            row=1, column=1, sticky="W", padx=110, pady=5
        )
        ...
        self.save_reference_select_image_entry = tk.Entry(
            self.reference_select_image_button_frame, font="Helvetica, 14"
        )
        ...
        self.product_open_camera_button.grid(
        ...
        self.product_close_camera_button.grid(
        ...
        self.product_compare_image_button.grid(
            row=0, column=2, sticky="W", padx=10, pady=10
        )

        # Label
        self.result_static_label = tk.Label(
            self.result_label_frame, text="Ürün Kontrol Sonucu:", font="Helvetica, 22"
        )
        ...
        self.result_dynamic_label = tk.Label(
            self.result_label_frame, text="", font="Helvetica, 24", fg="#00FF00"
        )
    ...
    def create_new_window(self):
        params = read_last_reference_image_parameters(
    ...
    def initialize(self):
        if self.selected_reference_image_path.exists():
            ...
            paths.SRC_ASSETS_DIR.joinpath("reference_images_names.txt").touch(
                exist_ok=True
            )
            paths.SRC_ASSETS_DIR.joinpath("last_reference_image_used.txt").touch(
                exist_ok=True
            )
    ...
    def redraw_saved_areas(self):
        ...
        saved_coords = read_last_reference_image_coordinates(
            self.selected_reference_image_name
        )
        for coords in saved_coords:
            rect_id = self.reference_canvas.create_rectangle(
                *coords, outline="#00FF00", width=2
            )
    ...
    def on_select_combobox(self, event):
        ...
        self.selected_reference_image.save(
            self.selected_reference_image_path, quality=95
        )
        self.reference_canvas.config(state="disabled")
        self.initial_components_status()
    ...
    def manage_reference_image_and_canvas(self, image_path):
        ...
        self.selected_reference_image_tk = ImageTk.PhotoImage(
            self.selected_reference_image
        )
        self.reference_canvas.create_image(
            0, 0, image=self.selected_reference_image_tk, anchor="nw"
        )

        self.selected_reference_image_coordinates = (
            read_last_reference_image_coordinates(self.selected_reference_image_name)
        )
    ...
    def filling_combobox_options(self):
        ...
        self.saved_reference_images_combobox["values"] = (
            self.saved_reference_images_names
        )
    ...
    def start_rect(self, event):
        if self.reference_canvas["state"] != "disabled":
            ...
            rect = self.reference_canvas.create_rectangle(
                self.start_x,
                self.start_y,
                self.start_x,
                self.start_y,
    ...
    def update_rect(self, event):
        if self.reference_canvas["state"] != "disabled":
            ...
            self.reference_canvas.coords(
                self.rects[-1], self.start_x, self.start_y, cur_x, cur_y
            )
    ...
    def reference_image_area_apply_button_click(self):
        all_selected_coordinates = (
            []
        )  # Seçilen tüm alanların koordinatlarını tutacak liste
        for rect in self.rects:
            coordinates = self.reference_canvas.coords(
                rect
            )  # Mevcut dikdörtgenin koordinatlarını al
            if coordinates:
                ...
                box = (
                    ...
                    int(x2),
                    int(y2),
                )  # Koordinatları bir tuple olarak sakla
                all_selected_coordinates.append(box)  # Koordinat setini listeye ekle
                self.reference_canvas.itemconfig(
                    rect, outline="#00FF00", width=2
                )  # Dikdörtgeni onaylanmış olarak işaretle
    ...
    def reference_select_image_button_click(self):
        confirmation = messagebox.askyesno(
            "Referans Görüntü Değiştirme",
            "Referans görüntü değiştirilecek. " "Devam etmek istiyor musunuz?",
            icon=messagebox.QUESTION,
        )
        if confirmation:
    ...
    def save_reference_select_image_button_click(self):
        ...
        if not self.new_reference_image_name:
            ...
            if self.selected_reference_image_path.exists():
                self.manage_reference_image_and_canvas(
                    self.selected_reference_image_path
                )
            else:
                self.initial_components_status_with_selected_image_check()
            return messagebox.showwarning(
                "Uyarı", "Lütfen referans görsel etiketi alanını boş bırakmayın!"
            )
        else:
            ...
            shutil.copy(self.reference_image_path, self.selected_reference_image_path)
            write_last_reference_image_coordinates(
                self.selected_reference_image_name, default_box
            )
            write_last_reference_image_parameters(
                self.selected_reference_image_name, default_values_of_image_parameters
            )
            ...
            messagebox.showinfo(
                "Başarılı",
                f"Referans görsel '{self.new_reference_image_name}' olarak kaydedildi!",
            )
            messagebox.showwarning(
                "Uyarı",
                f"Lütfen '{self.new_reference_image_name}' referans görseli için "
    ...
    def manage_product_image_and_canvas(self):
        ...
        self.current_image_on_canvas = self.product_canvas.create_image(
            0, 0, image=self.product_image_tk, anchor="nw"
        )

        for coords in self.selected_reference_image_coordinates:
            x1, y1, x2, y2 = coords
            self.product_canvas.create_rectangle(
    ...
    def manage_diff_image_and_canvas(self, diff_image, coords):
        ...
        self.product_canvas.itemconfig(
            self.current_image_on_canvas, image=self.product_image_tk
        )
    ...
    def minio_and_database_connection(self, brand_name, result_flag):
        ...
        if response.status_code == 200:
            ...
            upload_to_minio(
                record_id, brand_name, self.selected_reference_image_path, result_flag
            )

    def product_compare_image_button_click(self):
        if self.current_canvas is None:
            self.manage_product_image_and_canvas()
            cropped_reference_images, cropped_product_images = (
                self.crop_areas_to_compare_from_images()
            )
            ...
            for index, (cropped_reference_image, cropped_product_image) in enumerate(
                ...
                if not score:  # Eğer skor yoksa, yani eşleşme başarısızsa
                    coords = self.selected_reference_image_coordinates[
                        index - 1
                    ]  # İlgili koordinat setini al
                    self.manage_diff_image_and_canvas(diff_image, coords)

            result_text, result_color, result_flag = (
                ("BAŞARILI", "#00FF00", True)
                if score
                else ("BAŞARISIZ", "#ff1e00", False)
            )
    ...
    def crop_areas_to_compare_from_images(self):
        ...
        for rect in self.rects:
            ...
            if coordinates:
                ...
                cropped_reference_image = self._crop_image(
                    self.selected_reference_image, (x1, y1, x2, y2)
                )
                cropped_reference_images.append(cropped_reference_image)
                cropped_product_image = self._crop_image(
                    self.product_image, (x1, y1, x2, y2)
                )
    ...
    def update_image(self):
        if self.current_canvas:
            ...
            if ret:
                ...
                self.current_canvas.create_image(
                    0, 0, anchor="nw", image=self.update_image_tk
                )
    ...
    def reference_product_image_selection_enable_states(self):
        ...
        self.reference_image_area_select_button.config(
            state="normal", style="Accent.TButton"
        )
        self.reference_select_image_button.config(
            state="normal", style="Accent.TButton"
        )
        self.save_reference_select_image_entry.config(state="disable")
        self.save_reference_select_image_button.config(
            state="disable", style="Disabled.TButton"
        )
        self.product_open_camera_button.config(state="normal", style="Accent.TButton")
        self.product_open_camera_button.config(state="normal", style="Accent.TButton")
        self.product_close_camera_button.config(
            state="disable", style="Disabled.TButton"
        )
    ...
    def reference_image_selection_buttons_states(self):
        ...
        self.product_open_camera_button.config(
            state="disable", style="Disabled.TButton"
        )
        self.product_close_camera_button.config(
            state="disable", style="Disabled.TButton"
        )
        self.product_compare_image_button.config(
            state="disable", style="Disabled.TButton"
        )
    ...
    def reference_image_area_select_buttons_state(self):
        ...
        self.reference_image_area_clear_button.config(
            state="normal", style="Accent.TButton"
        )
        self.reference_image_area_apply_button.config(
            state="normal", style="Accent.TButton"
        )
        self.reference_select_image_button.config(
            state="disabled", style="Disabled.TButton"
